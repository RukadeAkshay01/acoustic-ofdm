...
\begin{itemize}
    ...
    \item \fillin{Post-mid-term work: add what you did on the convolutional code,
    clock-offset estimation and correction, the two-device test and measurements, the
    test suite or the dashboard, or delete this item.}
    \item \textbf{Documentation:} individual mid-term report (Member~1: modem engine);
    \fillin{sections of this report you wrote}.
\end{itemize}
"""),
...
\begin{itemize}
    ...
    testing and dashboard); \fillin{sections of this report you wrote}.
\end{itemize}
"""),
]
...
\fillin{Add one sentence on the two-device result.} Results are published on an
...
\begin{itemize}
    ...
    \item \textbf{Two devices.} \fillin{Transmitter and receiver models, room, distance,
    volume}.
...
\fillin{convolutional code, 8\,dB} & \fillin{} & \fillin{} \\
\hline
\end{tabular}
...
\begin{tabular}{|l|c|c|c|c|c|c|}
\hline
\textbf{Setup} & \textbf{Distance} & \textbf{Decoded} & \textbf{Exact} & \textbf{Clock offset} & \textbf{EVM} & \textbf{Raw BER} \\
\hline
A $\rightarrow$ B, repetition-3 & 0.5\,m & \fillin{} & \fillin{} & \fillin{} & \fillin{} & \fillin{} \\
A $\rightarrow$ B, repetition-3 & 1\,m & \fillin{} & \fillin{} & \fillin{} & \fillin{} & \fillin{} \\
A $\rightarrow$ B, repetition-3 & 2\,m & \fillin{} & \fillin{} & \fillin{} & \fillin{} & \fillin{} \\
A $\rightarrow$ B, repetition-3 & 3\,m & \fillin{} & \fillin{} & \fillin{} & \fillin{} & \fillin{} \\
A $\rightarrow$ B, convolutional & 1\,m & \fillin{} & \fillin{} & \fillin{} & \fillin{} & \fillin{} \\
B $\rightarrow$ A, repetition-3 & 1\,m & \fillin{} & \fillin{} & \fillin{} & \fillin{} & \fillin{} \\
\hline
\end{tabular}
\caption{Two separate devices, 5 frames per row
(\fillin{from \texttt{run\_two\_device.py report}})}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Two-device result.} \fillin{Measured clock offset between the devices;
    is it stable, and does it flip sign when roles swap? How far does each code reach?
    Does it confirm the simulated clock-offset result?}
...
faster and far more robust in simulation. \fillin{One sentence on the two-device
result.}

\section{Limitations}
\begin{itemize}
    \item The strongest results (convolutional code, drift correction) are verified in
    simulation; over-the-air confirmation is \fillin{complete / pending}.
...
\textbf{Google Drive Link:} \fillin{PASTE YOUR VIDEO DEMO LINK HERE}
...
\section{Source Code}
\textbf{Google Drive Link (ZIP file):} \fillin{PASTE YOUR CODE ZIP LINK HERE}
